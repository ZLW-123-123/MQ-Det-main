...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        if "positive_map" in transposed_batch[1][0].fields():
            ...
            batched_pos_map = torch.zeros((nb_boxes, max_len), dtype=transposed_batch[1][0].get_field("positive_map").dtype)
            cur_count = 0
        ...
        if "positive_map_eval" in transposed_batch[1][0].fields():
            ...
            batched_pos_map = torch.zeros((nb_boxes, max_len), dtype=transposed_batch[1][0].get_field("positive_map_eval").dtype)
            cur_count = 0
            for v in transposed_batch[1]:
