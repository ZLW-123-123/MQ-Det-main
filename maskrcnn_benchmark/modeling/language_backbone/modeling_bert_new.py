...
from timm.models.layers import trunc_normal_ # 新增引用
...
# NEW: Vision Refiner Attention Class
class VisionRefinerAttention(nn.Module):
    '''
    Attention mechanism for Vision attending to Text (Vision Refinement).
    ...
    def forward(self, vision, text, attention_mask=None):
        """
        vision: (B, V, D)
        text: (B, T, D)
        attention_mask: (B, V, T) - 1 for valid connection, 0 for masked
        """
        vision = self.norm_q(vision)
        text = self.norm_kv(text)
        ...
        # sim: (B, H, V, T)
        sim = einsum('b h v d, b h t d -> b h v t', q, k) * self.scale
        
        if exists(attention_mask):
            # attention_mask: (B, V, T) -> (B, 1, V, T)
            mask = attention_mask[:, None, :, :]
            # Mask logic: 0 means mask (ignore), so fill with -inf
            sim = sim.masked_fill(mask == 0, -1e4)
            
        attn = sim.softmax(dim=-1)
        
        # out: (B, H, V, D)
        out = einsum('b h v t, b h t d -> b h v d', attn, v)
...
class GatedCrossAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        # 1. Vision -> Text Attention (Original)
        ...
        # 2. [NEW] Text -> Vision Attention (Vision Refinement)
        self.vision_refiner = VisionRefinerAttention(dim=dim, dim_head=dim_head, heads=heads)
        self.vision_norm = nn.LayerNorm(dim)

        # 3. [MODIFIED] Channel-wise Gating
        # Previous out_dim was 1, now it is 'dim' for finer control
            ...
            if cfg.VISION_QUERY.CONDITION_GATE:
                if cfg.VISION_QUERY.NONLINEAR_GATE:
                    if cfg.VISION_QUERY.NO_CAT:
                        self.attn_gate = FeedForward(dim=dim, mult=0.5, out_dim = dim)
                        torch.nn.init.constant_(self.attn_gate.linear2.weight, 0)
                    else:
                        self.attn_gate = FeedForward(dim=dim*2, mult=0.5, out_dim = dim)
                        torch.nn.init.constant_(self.attn_gate.linear2.weight, 0)
                else:
        ...
        if enable_ffn:
            ...
            if cfg.VISION_QUERY.FIX_ATTN_GATE == -1.0:
                # [MODIFIED] Channel-wise FFN gate
    ...
    def forward(
        ...
        # 1. [NEW] Text-Guided Vision Refinement
        # Use text to clean/refine vision features before they attend to text
        vision_refined = self.vision_refiner(vision, x, attention_mask=attention_mask)
        vision = self.vision_norm(vision + vision_refined) # Update vision feature (Residual + Norm)

        # 2. Vision -> Text Attention (Using refined vision)
        dropped_x = x
        ...
        if self.cfg.VISION_QUERY.FIX_ATTN_GATE != -1.0:
            ...
            if self.cfg.VISION_QUERY.CONDITION_GATE:
                if self.cfg.VISION_QUERY.NO_CAT or not (self.cfg.VISION_QUERY.NONLINEAR_GATE):
                    ...
                    # Concatenate to decide gate values
        ...
        # Apply gate (broadcasting works for channel-wise)
        ...
        # Return both updated text and updated vision
        return x, vision
...
class QVBertEncoder(BertEncoder):
    ...
    def forward(
        ...
        # db
        # ================= [核心修改 START] =================
        # 对齐 Vision Attention Mask 的长度和 CoOp 偏移
        # 目标: 让 mask 的 text 维度与 hidden_states.shape[1] 一致
        
        current_seq_len = hidden_states.shape[1] # e.g., 23
        adjusted_vision_mask = vision_attention_mask
        
        # 仅当存在 Vision Query 且 mask 不为空时处理
        if exists(vision) and vision_attention_mask is not None:
            B, V, L_mask = vision_attention_mask.shape
            prompt_len = 16
            
            # 1. 处理 CoOp 平移: [CLS, Text] -> [CLS, Prompt(zeros), Text]
            if prompt_len > 0:
                # 分离 CLS 和 Text
                mask_cls = vision_attention_mask[:, :, 0:1] # [B, V, 1]
                mask_text = vision_attention_mask[:, :, 1:] # [B, V, L-1]
                
                # 创建 Prompt Mask (全0，Vision Query 不应该去增强 Prompt)
                mask_prompt = torch.zeros((B, V, prompt_len), device=hidden_states.device, dtype=vision_attention_mask.dtype)
                
                # 拼接
                adjusted_vision_mask = torch.cat([mask_cls, mask_prompt, mask_text], dim=2)
            
            # 2. 处理动态长度截断: [B, V, L_extended] -> [B, V, current_seq_len]
            # 此时 adjusted_vision_mask 的长度可能比 current_seq_len 长 (因为 mask 原本是 256)
            mask_len = adjusted_vision_mask.shape[2]
            
            if mask_len > current_seq_len:
                # 截断
                vision_attention_mask = adjusted_vision_mask[:, :, :current_seq_len]
            elif mask_len < current_seq_len:
                # 补齐 (通常不会发生，除非 padding='longest' 计算有误，但为了安全)
                pad_len = current_seq_len - mask_len
                pad_zeros = torch.zeros((B, V, pad_len), device=hidden_states.device, dtype=vision_attention_mask.dtype)
                vision_attention_mask = torch.cat([adjusted_vision_mask, pad_zeros], dim=2)
        
        # ================= [核心修改 END] =================
        # ---------------------------------------------------
        # 3. 处理 Batched Pos Category Map (List of Tensors)
        # ---------------------------------------------------
        # adjusted_pos_category_map = batched_pos_category_map
        if batched_pos_category_map is not None:
            new_map_list = []
            for map_item in batched_pos_category_map:
                # map_item 通常是 Tensor，形状可能是 (L,) 或 (Num_Cats, L)
                # 我们假设它是 Tensor，并对最后一个维度 (序列长度) 进行操作
                if isinstance(map_item, torch.Tensor):
                    # A. 处理 CoOp 平移
                    if prompt_len > 0:
                        # 切分 [CLS] 和 [Text]
                        # 假设最后一维是序列长度
                        map_cls = map_item[..., 0:1]
                        map_text = map_item[..., 1:]
                        
                        # 插入 Prompt (全 False/0，因为 Prompt 不是类别词)
                        # 保持前面的维度形状，只改变最后一维
                        prompt_shape = list(map_item.shape)
                        prompt_shape[-1] = prompt_len
                        map_prompt = torch.zeros(prompt_shape, device=map_item.device, dtype=map_item.dtype)
                        
                        map_item = torch.cat([map_cls, map_prompt, map_text], dim=-1)
                    
                    # B. 处理动态长度截断
                    curr_len = map_item.shape[-1]
                    if curr_len > current_seq_len:
                        map_item = map_item[..., :current_seq_len]
                    elif curr_len < current_seq_len:
                        pad_shape = list(map_item.shape)
                        pad_shape[-1] = current_seq_len - curr_len
                        pad = torch.zeros(pad_shape, device=map_item.device, dtype=map_item.dtype)
                        map_item = torch.cat([map_item, pad], dim=-1)
                        
                    new_map_list.append(map_item)
                else:
                    # 如果不是 Tensor (极少见)，原样保留或报错
                    new_map_list.append(map_item)
            
            batched_pos_category_map = new_map_list
        # ================= [核心修改 END] =================
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # [MODIFIED] Pass and update vision features (Bidirectional Interaction)
            if i >= self.start_qv_layer_index and exists(vision):
                qv_index = i - self.start_qv_layer_index
                hidden_states, vision = self.qv_layer[qv_index](hidden_states, vision, vision_attention_mask, batched_pos_category_map)
