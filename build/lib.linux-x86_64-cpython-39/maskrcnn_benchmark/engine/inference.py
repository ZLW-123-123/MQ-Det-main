...
def create_queries_and_maps(labels, label_list, additional_labels = None, cfg = None, disable_print = False):
    ...
    # print(os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE))
    # print('1111111111',os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE))
    # db
    # if os.path.basename(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE) == "/HOME/pxyai/pxyai_0034/zzllww/models--bert-base-uncased/snapshots/86b5e0934494bd15c9632b12f734a8a67f723594/":
    ...
    if cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "/HOME/pxyai/pxyai_0034/zzllww/models--bert-base-uncased/snapshots/86b5e0934494bd15c9632b12f734a8a67f723594/":
        tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE)
        tokenized = tokenizer(objects_query, return_tensors="pt")
...
def inference(
        ...
        # db
        # ================= [NEW] 添加路由相关参数 =================
        subtasks_lora_data=None,
        subtasks_mean_feat=None,
        routing_tau=0.5
        # ==========================================================
):
    # convert to a torch.device for efficiency
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    # ================= [NEW] 路由模式调用 =================
                    if subtasks_lora_data is not None and subtasks_mean_feat is not None:
                        # 传入路由参数
                        output = model(
                            images, 
                            captions=captions, 
